...
const admin = {
  ...
  upsertAdminAccount: async (payload: {
    email: string;
    password?: string;
    role: "founder" | "admin" | "editor";
    isActive: boolean;
  }): Promise<void> => {
    await request("/api/admin/roles", { json: { action: "upsertAccount", ...payload } });
  },

  updateUserRole: async (userId: string, role: "learner" | "admin" | "founder"): Promise<void> => {
    await request("/api/admin/roles", { json: { action: "updateUserRole", userId, role } });
  },
};
