...
import { db, dbConfigured, ACHARYA_SLUG } from "./supabase";
    ...
    try {
      after(async () => {
        try {
          ...
          await db.from("farmer_ai_usage").insert(row);
        } catch (err) {
      ...
      buildRow()
        .then((row) => db.from("farmer_ai_usage").insert(row))
        .then((r) => {
