...
export default function ModuleEditorPage() {
  ...
  async function saveContent() {
    ...
    try {
      ...
      setMessage('Saved. Published sections now refresh on the main website immediately.');
  ...
  async function updateSectionStatus(sectionId: string, status: string) {
    try {
      await api.admin.updateSection(sectionId, { status });
      setMessage(status === 'published' ? 'Section published. Main website content will refresh immediately.' : 'Section saved as draft/review. It will not appear on the main website until published.');
      loadData();
    } catch (err) {
      console.error(err);
    }
    ...
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-muted mt-1">
          {sections.length} section{sections.length === 1 ? '' : 's'} · draft/review/publish status · translation coverage.
        </p>
      </div>
            ...
            <div className="p-4 space-y-2.5">
              <div className="grid sm:grid-cols-2 gap-2">
                <input
                  ...
                  onBlur={(e) => updateSectionTitle(section.id, 'title_hi', e.target.value)}
                  placeholder="Hindi title"
                  className="rounded-lg border border-line bg-cream px-3 py-2 text-xs"
                />
                <input
                  defaultValue={section.title_bn}
                  onBlur={(e) => updateSectionTitle(section.id, 'title_bn', e.target.value)}
