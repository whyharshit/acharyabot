...
3. Click any request that goes to `*.supabase.co` (e.g. `arjun_modules?select=*`).
...
console.log('LEARNERS (device IDs, phones, langs):',
  await q('arjun_learners', 'id,device_id,phone,preferred_lang,last_seen'));

console.log('CHAT LOGS (every question every user has ever asked Arjun):',
  await q('arjun_chat_logs', 'learner_id,lang,user_message,ai_response,created_at', 10));

console.log('QUIZ ATTEMPTS (which questions, which answers, how they scored):',
  await q('arjun_quiz_attempts', 'learner_id,module_id,score,total,created_at', 10));

console.log('PROGRESS (every learner × every module):',
  await q('arjun_progress', 'learner_id,module_id,sections_completed,completed', 10));

console.log('APPLY LOGS (field debrief self-assessments — real client names inside):',
  await q('arjun_apply_logs', 'learner_id,module_id,data,created_at', 10));

console.log('EVENTS (behavioural telemetry):',
  await q('arjun_events', 'learner_id,event_type,event_data,created_at', 10));
...
await fetch(`${SUPABASE_URL}/rest/v1/arjun_progress?learner_id=eq.PASTE_A_REAL_LEARNER_ID&module_id=eq.M01-intro`, {
  method: 'DELETE',
  headers: { apikey: ANON_KEY, Authorization: `Bearer ${ANON_KEY}` },
...
|---|---|---|---|---|
| `arjun_learners` | ✅ anyone | ✅ | ✅ | ✅ |
| `arjun_progress` | ✅ | ✅ | ✅ | ✅ |
| `arjun_quiz_attempts` | ✅ | ✅ | ✅ | ✅ |
| `arjun_apply_logs` | ✅ | ✅ | ✅ | ✅ |
| `arjun_chat_logs` | ✅ read-all | ✅ | ❌ | ❌ |
| `arjun_events` | ❌ | ✅ | ❌ | ❌ |
...
- Supabase enforces Row Level Security (RLS) based on the policies in `supabase/migrations/001_arjun_initial_schema.sql:186-193`.
...
DROP POLICY IF EXISTS "Anon manage learners"     ON arjun_learners;
DROP POLICY IF EXISTS "Anon manage progress"     ON arjun_progress;
DROP POLICY IF EXISTS "Anon manage quiz"         ON arjun_quiz_attempts;
DROP POLICY IF EXISTS "Anon manage earned_badges" ON arjun_earned_badges;
DROP POLICY IF EXISTS "Anon manage apply_logs"   ON arjun_apply_logs;
DROP POLICY IF EXISTS "Anon read own chat_logs"  ON arjun_chat_logs;

-- Re-add only the ones the client actually needs
CREATE POLICY "Anon insert learners"  ON arjun_learners      FOR INSERT WITH CHECK (true);
CREATE POLICY "Anon upsert progress"  ON arjun_progress      FOR INSERT WITH CHECK (true);
CREATE POLICY "Anon update progress"  ON arjun_progress      FOR UPDATE USING (true) WITH CHECK (true);
CREATE POLICY "Anon insert quiz"      ON arjun_quiz_attempts FOR INSERT WITH CHECK (true);
CREATE POLICY "Anon insert apply"     ON arjun_apply_logs    FOR INSERT WITH CHECK (true);
CREATE POLICY "Anon insert badges"    ON arjun_earned_badges FOR INSERT WITH CHECK (true);
...
> - `ModuleLoader` and admin dashboards read from content tables only (`arjun_modules`, `arjun_sections`, `arjun_content`, `arjun_videos`) — those SELECT policies stay.
