...
export function Avatar({ size = 48, ring = true, useImage = false, className = "" }: AvatarProps) {
  ...
  const style: CSSProperties = {
    width: size,
    height: size,
    background: "radial-gradient(circle at 35% 30%, #67C7F2, var(--color-forest-deep) 75%)",
  ...
  return (
    <div
      ...
      style={style}
      aria-label="Vajra Acharya"
      data-image-avatar={useImage ? "true" : "false"}
    >
      <span style={{ transform: "translateY(-2%)" }}>V</span>
    </div>
  );
}
