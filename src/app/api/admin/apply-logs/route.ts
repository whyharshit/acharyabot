...
export async function GET(req: NextRequest) {
  ...
  let countQ = db.from("vajra_acharya_apply_logs").select("*", { count: "exact", head: true });
  let rowsQ = db.from("vajra_acharya_apply_logs").select("*").order("created_at", { ascending: false });
