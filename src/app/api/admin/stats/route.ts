...
export async function GET() {
  ...
  const stats = await memoCache("admin:stats", 30, async () => {
    const [modules, sections, content, videos, learners, quizzes] = await Promise.all([
      db.from("vajra_acharya_modules").select("id", { count: "exact", head: true }),
      db.from("vajra_acharya_sections").select("id", { count: "exact", head: true }),
      db.from("vajra_acharya_content").select("id", { count: "exact", head: true }),
      db.from("vajra_acharya_videos").select("id", { count: "exact", head: true }),
      db.from("vajra_acharya_learners").select("id", { count: "exact", head: true }),
      db.from("vajra_acharya_quiz_attempts").select("id", { count: "exact", head: true }),
    ]);
    return {
